  ...
  try {
    ...
    DataSource.findOne({ _id: dataSource._id })
      .then((resDataSource) => {
        return (
          !!resDataSource &&
          resDataSource.headings.sort().toString() ===
            Object.keys(req.body.data).sort().toString() &&
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          const cellStyle =
            "border: 1px solid #000;padding: 5px 10px;text-align: left;";
          let html = `
...
exports.getAll = (req, res) => {
  Data.find({ userId: req.userId, dataSourceId: req.params.dataSourceId })
    ...
    .then(async (datas) => {
      ...
      return [dataSource.headings, datas];
    })
    .then(([headings, datas]) => {
      res.json({
        success: true,
